...
enum SessionCheckInSchedule {
    ...
    static let firstCheckInDelay: TimeInterval = 90 * 60
    ...
    static func checkInDates(startsAt: Date, endsAt: Date, now: Date = .now) -> [Date] {
        var dates: [Date] = []
    ...
    static func hasRoomForEscalation(after checkInCount: Int) -> Bool {
...
enum SaferPlanReminderSchedule {

    /// How long before a plan ends each reminder lands.
    static let offsets: [TimeInterval] = [60 * 60, 30 * 60, 10 * 60]
    ...
    static let legacyLabels = ["1 hour", "30 minutes", "10 minutes"]
