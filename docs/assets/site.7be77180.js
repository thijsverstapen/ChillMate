...
(function () {
  ...
  function initWalk() {
    ...
    // Only the frame currently on screen is ever downloaded, which is the right
    // default and has one sharp edge: scroll quickly and the next frame is
    // revealed before its bytes have arrived, so the phone flashes empty. Warm
    // the neighbours instead. They are lazy images, so all this does is start
    // the fetch a moment before it is needed rather than a moment after.
    function warm(index) {
  ...
  function initDemo() {
    ...
    if (fromHash()) {
      render();
      // Scrolling to the checker is right when it is the fourth chapter of a
      // long page. On its own page the checker *is* the page, and scrolling
      // pushes the heading off the top so a shared link opens on a fragment of
      // a form with no title above it. Only move if there is something to
      // move past.
      var above = root.getBoundingClientRect().top + window.scrollY;
      if (above > window.innerHeight * 0.6) {
